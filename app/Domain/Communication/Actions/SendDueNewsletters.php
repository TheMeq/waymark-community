...
final readonly class SendDueNewsletters
{
    ...
    {
        $sent = 0;
        Newsletter::query()->where('status', 'scheduled')->where('scheduled_for', '<=', now())->orderBy('scheduled_for')->each(function (Newsletter $newsletter) use ($actor, &$sent): void { $sent += $this->send->handle($actor, $newsletter); });
        return $sent;
    }
}
