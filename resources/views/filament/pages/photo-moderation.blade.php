    ...
    <div class="space-y-4">
        ...
        <section class="space-y-3" aria-labelledby="photo-reports">
            ...
            @forelse ($reports as $report)
                ...
                <article class="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-amber-200 p-3 dark:border-amber-800">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Reported by {{ $report->reporter?->publicDisplayName() ?? 'Anonymous' }}</p>
        ...
        <section class="space-y-3" aria-labelledby="photo-removal-requests">
            ...
            @forelse ($removalRequests as $request)
                ...
                <article class="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-amber-200 p-3 dark:border-amber-800">
                    <div class="flex items-center gap-3">@if ($preview)<img src="{{ $preview->url }}" alt="{{ $preview->alt }}" class="h-16 w-20 rounded-lg object-cover" style="{{ $request->photo->presentationRotationStyle() }}" />@endif<div><p class="font-semibold">Uploader removal request</p><p class="text-sm text-gray-600">{{ $request->photo?->caption ?: 'Community photo' }} · {{ $request->photo?->event?->title ?? $request->photo?->specialAlbum?->title }}</p><p class="mt-1 text-sm text-gray-600">Requested by {{ $request->requester?->publicDisplayName() }}</p>@if ($request->detail)<p class="mt-1 text-sm">{{ $request->detail }}</p>@endif</div></div>
