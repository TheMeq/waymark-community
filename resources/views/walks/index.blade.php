    ...
    <section class="bg-surface-raised py-10 sm:py-14">
        <div class="wm-container">
            ...
            <form class="mt-8 grid gap-3 rounded-[var(--wm-radius-md)] border border-border bg-surface p-4 sm:grid-cols-2 lg:grid-cols-4" method="get" action="{{ route('walks.index') }}">
                ...
                <div><label class="text-sm font-medium" for="leader">Leader</label><select class="wm-form-control mt-1" id="leader" name="leader"><option value="">Any leader</option>@foreach ($leaders as $leader)<option value="{{ $leader->id }}" @selected($filters->leaderId === $leader->id)>{{ $leader->name }}</option>@endforeach</select></div>
