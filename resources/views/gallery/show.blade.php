...
@section('content')
    <article class="wm-container py-[var(--wm-space-7)] max-w-5xl">
        <img src="{{ $photo->imageUrl }}" alt="{{ $photo->caption ?? 'Community photo' }}" width="{{ $photo->width }}" height="{{ $photo->height }}" class="w-full rounded-[var(--wm-radius-lg)] bg-surface-raised object-contain shadow-[var(--wm-shadow-card)]">
