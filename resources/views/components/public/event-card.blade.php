...
<article {{ $attributes->class('group overflow-hidden rounded-[var(--wm-radius-md)] border border-border bg-surface-raised shadow-[var(--wm-shadow-card)]') }}>
    <a class="block overflow-hidden" href="{{ $event['url'] }}" tabindex="-1" aria-hidden="true">
        <img
            class="aspect-[16/7] w-full object-cover transition-transform duration-300 group-hover:scale-[1.025]"
            src="{{ $event['image_url'] }}"
            alt="{{ $event['image_alt'] }}"
            loading="lazy"
        >
    ...
    <div class="p-3">
        ...
        <p class="mt-1 text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-brand">{{ $event['date'] }}</p>
